...
Deno.test(
  "DenoDownloader - downloads latest version",
...
Deno.test(
  "DenoDownloader - downloads specific version",
  { ignore: Deno.env.get("RUN_DOWNLOADER_TESTS") ? false : true },
  async () => {
    await withTempDir(async (tempDir) => {
      const version = "2.6.7";
...
Deno.test(
  "DenoDownloader - caches downloaded binary",
  { ignore: Deno.env.get("RUN_DOWNLOADER_TESTS") ? false : true },
  async () => {
    await withTempDir(async (tempDir) => {
      const version = "2.6.6";
...
Deno.test(
  "DenoDownloader - cleans up old versions",
  { ignore: Deno.env.get("RUN_DOWNLOADER_TESTS") ? false : true },
  async () => {
    await withTempDir(async (tempDir) => {
      const downloader = new DenoDownloader({ baseDir: tempDir });
...
Deno.test("DenoDownloader - handles different platforms", {
  ignore: Deno.env.get("RUN_DOWNLOADER_TESTS") ? false : true,
}, async () => {
  await withTempDir(async (tempDir) => {
