...
/*
  Opentofu binaries can be downloaded from:
...
export class OpenTofuDownloader implements Downloader {
  ...
  private async getLatestVersion(): Promise<string> {
    ...
    const response = await fetch("https://api.github.com/repos/opentofu/opentofu/releases", { headers });
    if (!response.ok) {
      throw new Error(`Failed to fetch OpenTofu releases: ${response.statusText}`);
    }
    ...
    return gaReleases[0].tag_name.replace(/^v/, "");
  }
  ...
  private async downloadIfNeeded(version: string): Promise<string> {
    ...
    const binaryName = platform === "windows" ? "tofu.exe" : "tofu";
    ...
    console.log(`Downloading OpenTofu ${version} from ${downloadUrl}`);
    ...
    if (!response.ok) {
      throw new Error(`Failed to download OpenTofu: ${response.statusText}`);
    }

    const zipData = new Uint8Array(await response.arrayBuffer());
    const zipPath = join(versionDir, "tofu.zip");
  ...
  private async verifySha256(
    ...
    const checksumUrl = `https://github.com/opentofu/opentofu/releases/download/v${version}/tofu_${version}_SHA256SUMS`;
  ...
  private async cleanupOldVersions(): Promise<void> {
    try {
      ...
      console.warn(`Failed to cleanup old OpenTofu versions: ${error}`);
    }
  }
